...
async def main() -> int:
    ...
    if not dns_ok.get("api.nobitex.ir") and not doh_ok.get("api.nobitex.ir"):
        ...
        print("   ایران پاسخ DNS نمی‌دهد. این با «رد کردن» فرق دارد و")
        print("   با عوض کردن DNS سرور هم درست نمی‌شود.\n")
        print(f"   {B}کاری که باید بکنید — یک بار:{END}\n")
        print(f"   {DIM}⚙️ سیستم ← 💳 راه‌های پرداخت ← 💱 نرخ تتر{END}")
        print("   نرخ تتر را بگذارید. ترون از قیمت جهانی‌اش ضربدر همین")
        print("   حساب می‌شود و خودکار می‌ماند.\n")
        print(f"   {DIM}تتر کند حرکت می‌کند و ترون تند؛ پس آن که فراموش کردنش")
        print(f"   ضرر می‌زند، همان است که خودکار مانده.{END}\n")
        return 1
