...
def bot():
    """رباتِ متصل، برای بخش‌هایی که باید <b>با آن بفرستند</b> نه فقط
    هشدار بدهند — مثل حالت ساده، که خودِ ربات در مقصد پست می‌گذارد.

    None یعنی ربات هنوز بالا نیامده؛ صداکننده باید خودش تصمیم بگیرد،
    چون برای هشدار «نرفت» قابل قبول است و برای ارسالِ پست نیست.
    """
    return _bot
