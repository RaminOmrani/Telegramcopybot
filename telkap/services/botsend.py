...
def to_bot_entities(entities) -> list:
    ...
    for entity in entities or ():
        ...
        if kind == "text_link":
            ...
            #
            # <b>این شاخه امروز اثری ندارد — و عمداً می‌ماند.</b>
            #
            # تلگرام اجازه‌ی فرستادنِ ایموجی پریمیوم را فقط به رباتی
            # می‌دهد که روی Fragment یوزرنیمِ کلکسیونی خریده باشد. راهِ
            # دومش — «صاحبِ ربات پریمیوم دارد» — فقط در چت خصوصی و گروه
            # و سوپرگروه کار می‌کند و <b>کانال در آن فهرست نیست</b>؛
            # یعنی برای ما که مقصدمان همیشه کانال است، بی‌فایده.
            #
            # پس امروز تلگرام همان ایموجیِ معمولیِ زیرش را نشان می‌دهد و
            # پست سالم می‌رسد. ولی روزی که یوزرنیم Fragment به ربات
            # وصل شود، همین خط بدون هیچ تغییری زنده می‌شود — پس حذفش
            # به‌عنوان «کد مرده» یعنی آن روز دوباره باید نوشته شود.
        elif kind == "pre":
