...
class DeliveryTiming(Base):
    ...
    VIA_SIMPLE = "simple"           # پویشِ مبدأ عمومی در حالت ساده
    ...
    #
    # <b>و `simple` هم عمداً بیرون است.</b> `sweep_share` برای این ساخته
    # شد که بگوید «آپدیت‌ها نمی‌رسند» — یک خرابی. ولی در حالت ساده
    # آپدیتی در کار نیست و پویش <b>خودِ طراحی</b> است؛ ریختنش در همان
    # کاسه، آن هشدار را با رشدِ حالت ساده بی‌معنا می‌کند.
